//
//  YearTransitionShareSheet.swift
//  Finite
//
//  SST §18.5: Share sheet for Year Transition (birthday week)
//  Appears when user opens app during their birthday week
//  Includes quote picker for personal reflection
//

import SwiftUI

struct YearTransitionShareSheet: View {
    let user: User
    let age: Int
    ...
    private var activeQuote: String? {
        if isUsingCustomQuote {
            return customQuote.isEmpty ? nil : customQuote
        }
        return selectedQuote
    }

    init(user: User, age: Int) {
        self.user = user
        self.age = age
        _selectedQuote = State(initialValue: BirthdayQuotes.random(for: age))
    }
    ...
    private var headerSection: some View {
        VStack(spacing: 8) {
            Text("Happy birthday week 🎂")
                .font(.title3.weight(.medium))
                .foregroundStyle(Color.textPrimary)
                .multilineTextAlignment(.center)
                .padding(.top, 24)

            Text("You're entering year \(age) of your life.")
                .font(.subheadline)
                .foregroundStyle(Color.textSecondary)
        }
        .padding(.bottom, 16)
    ...
    private var quoteSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(BirthdayQuotes.defaults(for: age), id: \.self) { quote in
                        quoteChip(quote)
                    }
    ...
    private func exportAndShare() {
        ...
        Task { @MainActor in
            if let image = YearTransitionCardExporter.exportImage(
                age: age,
                weekNumber: user.currentWeekNumber,
                quote: activeQuote,
                style: selectedStyle
                ...
                exportedImage = image
                showShareSheet = true
                shareFlow.onShareCompleted(type: .yearTransition)
...
#Preview {
    ...
    return YearTransitionShareSheet(user: user, age: 30)
}
